//Encode an ASCII string into its location in the Polybius square
...
//Decode a string of ints into their corresponding ASCII characters
//Any invalid characters are ignored
pub fn decode_ascii(string: &str) -> String {
    string.chars().filter(|c| !c.is_whitespace()).collect::<String>().as_bytes().chunks(2).map(|s| match std::str::from_utf8(s) {
